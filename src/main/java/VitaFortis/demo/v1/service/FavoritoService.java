...
@Service public class FavoritoService {
 ...
 @Transactional(readOnly=true) public List<ProdutoResponseDto> listar(Long id,String email){autorizado(id,email);return favoritos.findByUsuarioIdOrderByCriadoEmDesc(id).stream().map(Favorito::getProduto).filter(Produto::isAtivo).map(mapper::toResponseDto).toList();}
