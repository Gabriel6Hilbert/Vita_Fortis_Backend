...
export function CollaboratorPage() {
  ...
  return <div className="page container collaborator-page">
    ...
    {error ? <ErrorState message={error} retry={load} /> : loading || !data ? <Loading /> : <>
      <section className="chart-card"><h2>Meus cupons</h2><div className="table-wrap"><table><thead><tr><th>Cupom</th><th>Desconto</th><th>Cashback</th><th>Validade</th><th>Usos</th><th>Situação</th></tr></thead><tbody>{data.cupons.map(c=><tr key={c.id}><td>{c.codigo}</td><td>{c.tipo==='PERCENTUAL'?c.desconto+'%':money(c.desconto)}</td><td>{c.percentualCashback}%</td><td>{c.dataInicio?date(c.dataInicio):'Imediata'} até {c.dataVencimento?date(c.dataVencimento):'sem vencimento'}</td><td>{c.pedidos} / {c.limiteUso??'sem limite'}</td><td>{!c.ativo?'Inativo':c.limiteUso&&c.pedidos>=c.limiteUso?'Esgotado':c.dataVencimento&&new Date(c.dataVencimento)<=new Date()?'Expirado':c.dataInicio&&new Date(c.dataInicio)>new Date()?'Agendado':'Ativo'}</td></tr>)}</tbody></table>{!data.cupons.length&&<p>Nenhum cupom vinculado.</p>}</div></section>
